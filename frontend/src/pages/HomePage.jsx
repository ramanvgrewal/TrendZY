...
import { groupByParentCategory } from '../utils/productTypeClassifier';
...
export default function HomePage() {
    ...
    const { data: trendingData, isLoading: trendingLoading, isError: trendingError, refetch: refetchTrending } =
        useQuery({
            ...
            queryFn: () => getTrends('trending', vibeParam, 0, 100),
        });

    const { data: risingData, isLoading: risingLoading, isError: risingError, refetch: refetchRising } =
        useQuery({
            queryKey: ['rising', activeVibe],
            queryFn: () => getTrends('rising', vibeParam, 0, 100),
        });

    const trendingGroups = useMemo(() => groupByParentCategory(trendingData), [trendingData]);
    const risingGroups = useMemo(() => groupByParentCategory(risingData), [risingData]);
            ...
            <main style={{ flex: 1 }}>
                    ...
                    <span className="font-mono" style={{
                      ...
                      #1 Trending
                                    ...
                                    <div className="font-mono" style={{
                                        ...
                                        {hero.category && hero.estimatedPrice > 0 && (
                                            <span style={{ color: 'rgba(255,255,255,0.25)' }}>·</span>
                                        )}
                                        {hero.estimatedPrice > 0 && (
                                            <span style={{ color: '#f5f5f5' }}>
                        Est. ₹{hero.estimatedPrice.toLocaleString('en-IN')}
                      </span>
                                        )}
                                    </div>
                        ...
                        {statsLoading ? (
                            ...
                            <>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                                    ...
                                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', color: '#a3e635', textTransform: 'uppercase' }}>
                    Live Engine
                  </span>
                                </div>
                                {[
                                    ['Signals', stats.signalsToday],
                                    ['Trending', stats.trendingCount],
                                    ['Rising', stats.risingCount],
                                    ['Subreddits', stats.subredditsMonitored],
                                ].map(([label, value]) =>
                                        value ? (
                ...
                <div style={{ paddingTop: 32, paddingBottom: 4 }}>
                    <SectionDivider
                        ...
                        subtitle="Highest velocity signals from Reddit & YouTube — browse by what you're looking for."
                        badgeColor="#a3e635"
                        live
                    />
                    {trendingLoading ? (
